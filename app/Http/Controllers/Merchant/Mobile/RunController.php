...
class RunController extends Controller {
    ...
    public function actionSave(Request $request) {
        ...
        $orderinfo = BookingOrder::where(['out_trade_no' => $request->order_no, 'hotel_id' => \Auth::guard('run')->user()->hotel_id])->first();
        if (!$orderinfo) {
        ...
        if ($confirm_type == 1) {
            ...
            BookingOrder::orderCancel($request->order_no);
        }
    ...
    public function viewBookingOrder(Request $request){
        ...
        return view('merchant.mobile.run.viewBookingOrder', compact('info'));
    }
}
