...
class NotifyController extends AController {
    ...
    public function bookingOrder($message){
        ...
        if ($orderinfo->status != 2) {
            ...
            // 添加到店核销码
            \App\Models\Hotel\TicketsCode::waibuAddCode($orderinfo->hotel_id,\App\Models\Hotel\Order\Order::Sign_HotelRoomBooking,$out_trade_no,$orderinfo->code);
    ...
    private function rechargeHandle($message) {
        ...
        // 更新用户信息
        \App\User::addBalance($orderinfo->user_id, $full_price, '充值-微信支付');

        // 奖励积分
        \App\User::addPoint($orderinfo->user_id, 1, '充值-微信支付');
        return true;
    }
    ...
    public function tuangouOrder($message) {
        ...
        $pay_time = !empty($message['time_end']) ? $message['time_end']:$message['success_time'];
        $dateTime = new \DateTime($pay_time);
        ...
        // 进行分账
        //(new ProfitsharingService())->profitsharing($message['out_trade_no']);
    ...
    /*// 团购服务商品
    public function tuangouOrder($message){
        $orderinfo = \App\Models\Hotel\Order\Order::where(['order_no' => $message['out_trade_no']])->first();
        if (empty($orderinfo->order_no)) {
            return false;
        }
        $date          = \DateTime::createFromFormat('YmdHis', $message['time_end']);
