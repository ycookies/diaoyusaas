...
class TotalBookingRoom extends Card
{
    /**
     * 卡片底部内容.
     *
    ...
    public function handle(Request $request)
    {
        switch ($request->get('option')) {
            ...
            case '7':
            default:
                $totals = \DB::connection('hotel')->table('booking_order')->where([['hotel_id','>',100],['pay_status','=',1]])->groupBy('room_id')->select('room_id')->get();

                $this->content(count($totals));
                $this->up(15);
        }
    }
