...
// 列表
class HotelKefucenterController extends AdminController
{
    ...
    protected function grid()
    {
        return Grid::make(Kefucenter::with('hotel'), function (Grid $grid) {
            ...
            $grid->column('id');
            $grid->column('msg_info','酒店/平台/用户')->display(function (){
                $info  = $this->hotel->name.'<br/><span class="text-muted">'.$this->platform_str.'<br/>'.$this->user_openid.'</span>';
                return $info;
            });
            ...
            $grid->column('msg_to','作者')->using(Kefucenter::Msg_to);
            $grid->column('msg_type')->using(Kefucenter::$msg_type_arr);
            $grid->column('msg_content')->display(function (){
...
HTML;
            ...
            $grid->filter(function (Grid\Filter $filter) {
                $filter->equal('hotel_id')->select(Hotel::all()->pluck('name','id'));
