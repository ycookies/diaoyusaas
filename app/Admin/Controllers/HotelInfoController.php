 ...
 * 酒店基础信息管理
 */
class HotelInfoController extends AdminController {
    /**
     * 指定翻译文件名称
     *
     * @var string
    ...
    public function getForm(){
        if (!empty($this->hotel_id)) {
            $form           = Form::make(new SellerModels())->edit($this->hotel_id);
            $form->action(url('/merchant/storeinfo/' . $this->hotel_id));
    ...
    public function tab2() {
        $infos = SellerModels::where(['id' => $this->hotel_id])->first();
        if ($infos) {
            $this->shopinfo = $infos;
            $this->hotel_user_id = $infos->hotel_user_id;
        ...
        //$form->hidden('hotel_user_id')->value($infos->hotel_user_id);
        ...
        $form->text('link_tel','联系电话')->required();
    ...
    public function tab3() {
        ...
        $form->disableEditingCheck();
        $form->disableViewCheck();
        $form->hidden('hotel_user_id')->value($this->hotel_user_id);
        $form->image('ewm_logo','酒店logo')->width(3)->disable()->help('建议尺寸大小:541*500')->saveFullUrl()->url('/upload/imgs')->autoUpload()->removable(false)->autoSave(false)->required();
        $form->multipleImage('img', '酒店场景图')->disable()->help('建议尺寸大小:220*180')->saveFullUrl()->url('/upload/imgs')->autoUpload()->removable(false)->autoSave(false)->required();
        $form->confirm('您确定要现在保存吗？', 'content');
    ...
    public function tab4() {
        ...
        $form->column(6, function (Form $form) {
            ...
            $form->width(4)->time('early_arrival_time','最早入店')->required();
            $form->width(4)->time('late_arrival_time','最晚入店')->required();
            $form->width(4)->time('early_departure_time','最早离店')->required();
            $form->width(4)->time('late_departure_time','最晚离店')->required();
            $form->html('<h3>退订&取消</h3>');
            $form->width(4)->select('non_cancelling_time','不可取消时间')->options(SellerModels::$non_cancelling_time_arr)->required();
            $form->width(9)->text('non_cancelling_explain','不可取消说明')->required();
            $form->textarea('rule','退订规则')->rows(3)->required();
        });
        $form->column(6, function (Form $form) {
    ...
    public function tab5() {
        ...
        $form->editor('service_facility', '更多设施');
        $form->confirm('您确定要现在保存吗？', 'content');
        $form->disableFooter();
        $Card = Card::make('', $form);
    ...
    public function tab10() {
        ...
        $form->url('video_url','宣传片视频');
        $form->image('video_cover','视频封面')->disable();
        $form->disableFooter();
        ...
        /*$form->photo('video_cover', '视频封面')
            ->width(6)
            ->disk('hotel_' . $this->hotel_user_id)
            ->accept('jpg,png,jpeg')
