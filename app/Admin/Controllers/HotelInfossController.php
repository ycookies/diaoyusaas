...
// 列表
class HotelInfossController extends AdminController
{
    ...
    {
        return Grid::make(new HotelInfo(), function (Grid $grid) {
    ...
    {
        return Show::make($id, new HotelInfo(), function (Show $show) {
            $show->field('id');
            $show->field('type');
    ...
    {
        return Form::make(new HotelInfo(), function (Form $form) {
            $form->display('id');
            $form->text('type');
            $form->text('seller_id');
            $form->text('user_id');
