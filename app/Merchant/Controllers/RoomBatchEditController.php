...
// 小程序
class RoomBatchEditController extends Controller {
    ...
    public function pageMain() {
        ...
        $tab->addLink('房价设置', admin_url('room-price-set'));
    ...
    public function tab1() {
        ...
        $form->width(10, 2);
        $form->action('room-batch-edit-save');
        ...
        $form->checkbox('room_ids', '选择房型')->options(RoomModel::where(['hotel_id' => Admin::user()->hotel_id])->pluck('name', 'id'))
            ->canCheckAll()
            ->required();
    ...
    public function tab3() {
        ...
        $grid = Grid::make(RoomTiaojiaLog::with('room'), function (Grid $grid) {
            $grid->model()->where(['hotel_id' => Admin::user()->hotel_id])->orderBy('id', 'DESC');
            //$grid->column('id');
            $grid->column('room_ids_txt', '相关客房')->label();
            $grid->column('date_type', '节假日类型')->using(RoomTiaojiaLog::Batch_tiaojia_type_arr)->label();
            ...
            $grid->filter(function (Grid\Filter $filter) {
                $filter->panel();
                //$filter->expand();
                $filter->equal('date_type')->select(RoomTiaojiaLog::Batch_tiaojia_type_arr)->width(3);
    ...
    // 保存调价日志
    public function batchTiaojiaSave(Request $request) {
        $seller = Admin::user();
        $validator = Validator::make($request->all(),[
            'act_type'           => 'required',
        ...
        if(count(array_filter($request->room_ids)) <= 0){
            return JsonResponse::make()->error('请选择房型');
        }
        ...
        $mk = (new RoomTiaojiaService())->exeTiaojia($model->id); // 执行调价
