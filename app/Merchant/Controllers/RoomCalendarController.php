...
class RoomCalendarController extends AdminController
{
    public $week_arr = [
        ...
        $views = Grid::make(new Room(), function (Grid $grid) {
            ...
            foreach ($calendar_arr as $key => $item) {
                ...
                $grid->column($field_name,$label.' <br/> '.$days_str)->display(function ($r) use ($field_name){
                    ...
                    $pricebiao = Roomprice::getRoomDaysPrice($this->id,$days_date);
                    if(empty($pricebiao['mprice'])){
                        $pricebiao = [
                            'mprice' => formatFloats($this->price),
                            'open_status' => 1,
                            'booking_status' => 0,
                        ];
                    }
                    ...
                    $room_num = RoomModel::getRoomNum($this,$days_date); // 剩余房间
                    $zaocan_num = RoomModel::getZaocanNum($this); // 早餐数量

                    return RoomModel::showDaysPrice($field_name,$this->id,$price_arr,$room_num,$zaocan_num);
                });
            }
