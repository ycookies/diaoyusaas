...
use App\Merchant\Repositories\SmgOrdersGood;
use Dcat\Admin\Form;
use Dcat\Admin\Grid;
use Dcat\Admin\Show;
...
class SmgOrdersGoodController extends AdminController
{
    ...
    {
        return Grid::make(new SmgOrdersGood(), function (Grid $grid) {
    ...
    {
        return Show::make($id, new SmgOrdersGood(), function (Show $show) {
            $show->field('id');
            $show->field('order_id');
            $show->field('goods_id');
            $show->field('cid');
    ...
    {
        return Form::make(new SmgOrdersGood(), function (Form $form) {
            $form->display('id');
            $form->text('order_id');
            $form->text('goods_id');
            $form->text('cid');
