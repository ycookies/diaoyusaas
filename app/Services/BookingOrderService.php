...
class BookingOrderService extends BaseService {
    ...
    // 开始分账
    public function profitsharing($out_trade_no) {
        $where[]   = ['out_trade_no', '=', $out_trade_no];
    ...
    public function profitsharingQuery($profitsharing_no) {
        $orderinfo = ProfitsharingOrder::where(['profitsharing_no' => $profitsharing_no])->first();
        ...
        if (!empty($sharing['status']) && $sharing['status'] == 'FINISHED') {
            ...
            BookingOrder::where(['out_trade_no' => $orderinfo->order_no])->update(['profitsharing_after_price' => $profitsharing_after_price]);
