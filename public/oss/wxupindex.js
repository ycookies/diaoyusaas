var uploaderImageFilter = {title: "Image files", extensions: "jpg,jpeg,gif,png,bmp"};
...
// 没有成功回周函数的上传
function getUploader(btnid,filter,single = false,fileobj,field = 'filelist[]') { //这里定义创建upload实例化函数。接收三个参数，dom元素ID值，成功回调函数，文件过滤
    ...
    uploader.bind('FilesAdded', function (uploader, files) {
        ...
        plupload.each(files, function (file) {
            var htmls1 = '<li class="upfiled" ><div  id="' + file.id + '"><b></b>'
                ...
                + '</div></li>';
            if(single){
                $('#'+btnid).parents('.up-list').prepend(htmls1);
            }else{
                $('#'+btnid).parents('.up-list').prepend(htmls1);
            }
    ...
    uploader.bind('FileUploaded', function (up, file, info) {
        if (info.status == 200) {
            ...
            $('#' + file.id).html('');
            var imgshow = '<input type="hidden" name="'+field+'" value="' + url + '" /><span><img data-preview-src="'+url+'" data-preview-group="1"  src="' + url + '" onerror="imgErrorHandle(this)" /> </span><a href="javascript:;">删除</a>';
    ...
    uploader.bind('Error', function (up, err) {
        if (err.code == -600) {
            ...
            mui.alert("这个文件已经上传过一遍了");
        }else{
            mui.alert('上传发生错误:'+err.code);
        }
    });
}
...
function set_upload_param(up,filename, ret) {
    $("#img_url_id4").text('正在start1');
    if (ret == false) {
        ret = get_signature()
    }
    $("#img_url_id4").text('正在start2');
    g_object_name = key;
    if (filename != '') {
        suffix = get_suffix(filename)
        calculate_object_name(filename)
    }
    $("#img_url_id4").text('正在start3');
...
function wxUpimg(filename) {
    var uploader = new plupload.Uploader({
        browse_button: 'fileup145',// id值
        multi_selection: false,
        flash_swf_url: 'lib/plupload-2.1.2/js/Moxie.swf',
        silverlight_xap_url: 'lib/plupload-2.1.2/js/Moxie.xap',
        url: 'http://oss.aliyuncs.com',//上传路径一般不会变
        filters: {
            mime_types:[{title: "Image files", extensions: "jpg,jpeg,gif,png,bmp"}],  //选择对应的文件类型
            max_file_size: '1000mb', //最大只能上传10mb的文件
            prevent_duplicates: true //不允许选取重复文件
        }
    });
    uploader.init();
    $("#img_url_id3").text('正在初始化');
    set_upload_param(uploader,filename, false);
    $("#img_url_id3").text('上传开始');
    uploader.bind('UploadProgress', function (up, file) {
        $("#img_url_id3").text('正在上传');
    });
    uploader.bind('FileUploaded', function (up, file, info) {
        if (info.status == 200) {
            mui.alert('上传成功');
