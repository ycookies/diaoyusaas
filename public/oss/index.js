...
var uploaderImageFilterAll = [ //只允许上传图片和zip文件
    ...
    {title: "video files", extensions: "mp4,avi,rmvb,wma,rm,flash"}
];
accessid = ''
accesskey = ''
host = ''
...
// 有成功回周函数的上传
function getUploader(btnid, success, filter) { //这里定义创建upload实例化函数。接收三个参数，dom元素ID值，成功回调函数，文件过滤
    ...
    uploader.bind('FilesAdded', function (uploader, files) {
        ...
        plupload.each(files, function (file) {
            var htmls1 = '<div class="images_zone" id="' + file.id + '"><b></b>'
                ...
                + '</div>';
            $('#'+btnid).parents('.form-group').find('.file1').append(htmls1);
            g_object_name_type = 'local_name'; // 本地文件名，还是随机文件名
    ...
    uploader.bind('FileUploaded', function (up, file, info) {
        if (info.status == 200) {
            ...
            $('#' + file.id).html('');
            var imgshow = '<input type="hidden" name="filelist[]" value="' + url + '" /><span><img src="' + url + '" onerror="this.src=\'/assets/assets/images/l_load.png\'" /><div class="look_file" onclick="look_load(this)">查看下载</div></span><a href="javascript:;">删除</a>';
