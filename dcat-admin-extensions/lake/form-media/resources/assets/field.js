...
$(function () {
    var LakeFormMedia = {
            ...
            this.onEvent('change', '.lake-form-media-upload', function() {
                ...
                layer.load(3, {
                    ...
                    success: function(layerElem) {
                        // 设置 z-index 高于弹窗
                        $(layerElem).css('z-index', 199999999); // 19891015 是 Layui 弹窗的默认 z-index
                    }
                });
                $.ajax({
                    type: 'post', 
                ...
                success: function (res) {
                    ...
                    if (JSON.stringify(list) != '[]') {
                        for (var i in list) {
                            if (list[i]['isDir']) {
                                var htmltemp = '';
                                htmltemp += '<div class="col-xs-4 col-md-2">';
                                ...
                                htmltemp +=             '<a href="javascript:;" class="file-name tips" data-title="'+ list[i]['namesmall'] +'">'+list[i]['namesmall']+'</a>';
                                htmltemp +=         '</div>';
                                htmltemp +=     '</div>';
                                htmltemp += '</div>';
        ...
        refreshPreview: function(name, urlList, options = []) {
            ...
            var fieldIconimg = mediaCont.attr('data-field');
            for (var i = 0; i < urlList.length; i++) {
                ...
                if(fieldIconimg == 'iconimg'){
                    var html = '<div class="lake-form-media-preview-item" data-src="'+urlList[i]+'">';
